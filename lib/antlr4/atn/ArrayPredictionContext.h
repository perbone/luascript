...
namespace atn {
  ...
  public:
    ...
    const std::vector<__Ref<PredictionContext>> parents;

    /// Sorted for merge, no duplicates; if present, EMPTY_RETURN_STATE is always last.
    const std::vector<size_t> returnStates;

    ArrayPredictionContext(__Ref<SingletonPredictionContext> const& a);
    ArrayPredictionContext(std::vector<__Ref<PredictionContext>> const& parents_, std::vector<size_t> const& returnStates);
